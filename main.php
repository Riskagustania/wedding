  ...
  <div class="container" style="margin-top: 10rem;">
    <div class="row d-flex justify-content-center">
      <div class="col-6 d-flex justify-content-center">
        <a href="add_wedding.php" class="btn btn-outline-dark font-weight-bold" style="color: #1c5c5b;border-color: #1c5c5b;border-radius: 0; width: 110px;">Order Now</a>
      </div>
    </div>
    <div class="row">
      <?php foreach ($queryGetAllData as $data) : ?>
        <?php
        $idWedding = $data['wedding_id'];
        $namaPria = $data['nama_pria'];
        $usiaPria = $data['usia_pria'];
        $namaWanita = $data['nama_wanita'];
        $usiaWanita = $data['usia_wanita'];
        $tglPesta = $data['tanggal_pesta'];
        $alamatPesta = $data['alamat_pesta'];
        $budgetPesta = $data['budget'];
        ?>
        <div class="col-md-4">
          <div class="card bg-light my-3">
            <div class="card-header text-center">
              <h4><span style="color: #1c5c5b;"><?= $namaPria ?></span> <span style="color: red;">&#9829;</span> <span style="color: #ffa7ab;"><?= $namaWanita ?></span></h4>
            </div>
            <div class="card-body">
              <h6 class="card-text"><?= $namaPria ?> - <?= $usiaPria ?> tahun</h6>
              <h6 class="card-text"><?= $namaWanita ?> - <?= $usiaWanita ?> tahun</h6>
              <h6 class="card-text">Tanggal Pesta : <?= $tglPesta ?></h6>
              <h6 class="card-text">Alamat : <?= $alamatPesta ?></h6>
              <h6 class="card-text">Budget : Rp. <?= $budgetPesta ?>,-</h6>
              <div class="d-flex justify-content-around ">
                <form action="config/delete_wedding.php" method="POST">
                  <input type="hidden" name="edit_wedding_id" value="<?= $idWedding ?>">
                  <button type="submit" class="btn btn-outline-dark font-weight-bold" style="color: #ffa7ab; border-color: #ffa7ab; border-radius: 0; width: 110px;">Hapus</button>
                </form>
                <form action="edit_wedding.php" method="POST">
                  <input type="hidden" name="edit_wedding_id" value="<?= $idWedding ?>">
                  <input type="hidden" name="edit_nama_pria" value="<?= $namaPria ?>">
                  <input type="hidden" name="edit_nama_wanita" value="<?= $namaWanita ?>">
                  <input type="hidden" name="edit_usia_pria" value="<?= $usiaPria ?>">
                  <input type="hidden" name="edit_usia_wanita" value="<?= $usiaWanita ?>">
                  <input type="hidden" name="edit_tgl_pesta" value="<?= $tglPesta ?>">
                  <input type="hidden" name="edit_alamat_pesta" value="<?= $alamatPesta ?>">
                  <input type="hidden" name="edit_budget_pesta" value="<?= $budgetPesta ?>">
                  <button type="submit" name="edit" class="btn btn-outline-dark font-weight-bold" style="color: #1c5c5b; border-color: #1c5c5b; border-radius: 0; width: 110px;">Edit</button>
                </form>
              </div>
    ...
    <div class="row">
      <div class="col-12">
        <a href="#" style="text-transform: uppercase; color: #1c5c5b;" class="h1">Wedding
          service</a>
    ...
    <div class="container">
      <div class="row justify-content-center">
        ...
        <div class="col-6 d-flex flex-column">
          <a href="#" class="h1 mt-5" style="color: #1c5c5b;">Getting Ready</a>
  ...
  <div class="container mt-5 pt-5">
    <div class="row">
      <div class="col-5 d-flex flex-column">
        <a href="#" class="h1 mt-5" style="color: #1c5c5b;">Great camera</a>
